...
public class VoucherService {
    ...
    // ==========================================================
    // === CORE LOGIC: TÍNH TOÁN, XÁC THỰC VÀ GIẢM SỐ LƯỢNG ===
    // ==========================================================

    /**
     * XÁC THỰC VÀ TÍNH TOÁN GIẢM GIÁ (Đọc - readOnly)
     */
    @Transactional(readOnly = true)
    ...
    /**
     * 🔥 Phương thức chính: Giảm số lượng của Voucher đi 1 sau khi đơn hàng được tạo thành công.
     * Phương thức này cần được gọi từ OrderService.createOrder() (hoặc tương đương).
     * @param voucherCode Mã code của voucher cần giảm số lượng.
     * @throws RuntimeException nếu không tìm thấy voucher hoặc số lượng đã hết.
     */
    @Transactional // Đảm bảo hoạt động trong transaction của OrderService
    public void decreaseVoucherQuantity(String voucherCode) {
        if (!StringUtils.hasText(voucherCode)) {
            // Không áp dụng voucher, không làm gì cả
            return;
        }

        // 1. Tìm Voucher theo codeName
        Voucher voucher = voucherRepository.findByCodeNameIgnoreCase(voucherCode)
                .orElseThrow(() -> new RuntimeException("Voucher code '" + voucherCode + "' không tồn tại."));

        // 2. Kiểm tra số lượng
        Integer currentQuantity = voucher.getQuantity();
        if (currentQuantity == null || currentQuantity <= 0) {
            // Nên kiểm tra lại (re-validate) trước khi giảm, mặc dù đã kiểm tra ở calculateDiscount
            throw new RuntimeException("Voucher code '" + voucherCode + "' đã hết lượt sử dụng.");
        }
